...
- Choose the project-scoped custom agent from the routing table when its live model and reasoning match the route. Use the built-in fallback when the custom profile is unavailable or still exposes an older model or reasoning level after a configuration change; never override a fixed custom profile. Reload the session to pick up updated custom profiles.
...
| Bounded read-only analysis, alternatives, planning, or scoped architecture investigation | `code-general` | `gpt-5.6-luna` / `high` / read-only | `default`, `gpt-5.6-luna` / `high` |
| Cross-system or high-risk debugging (Astra main, Luna/Terra default, or justified escalation) | `code-debugger-astra` | `gpt-6-astra` / `high` / inherited permissions | `default`, `gpt-6-astra` / `high` |
| Cross-system or high-risk debugging (Sol main) | `code-debugger-sol` | `gpt-5.6-sol` / `high` / inherited permissions | `default`, `gpt-5.6-sol` / `high` |
| Explicit high-stakes security, regression, release, or final correctness gate (Astra main, Luna/Terra default, or justified escalation) | `code-reviewer-astra` | `gpt-6-astra` / `high` / read-only | `default`, `gpt-6-astra` / `high` |
| Explicit high-stakes security, regression, release, or final correctness gate (Sol main) | `code-reviewer-sol` | `gpt-5.6-sol` / `high` / read-only | `default`, `gpt-5.6-sol` / `high` |
| Configured milestone and final acceptance gate for an Astra main agent (also default for unrestricted Luna/Terra) | `code-gate-reviewer-astra` | `gpt-6-astra` / `max` / read-only | `default`, `gpt-6-astra` / `max` |
| Configured milestone and final acceptance gate for a Sol main agent | `code-gate-reviewer-sol` | `gpt-5.6-sol` / `max` / read-only | `default`, `gpt-5.6-sol` / `max` |

The live host schema is authoritative. For an ordinary route, execute directly when neither the custom profile nor its built-in fallback is exposed instead of silently selecting a mismatched agent. A mandatory model-matched gate is different: if no independent custom or built-in agent can be spawned, the verdict is `BLOCKED`; the main agent cannot replace the independent reviewer or claim `PASS`.

Route by purpose and risk, not by verbs in the request. A request to "review," "look at," or explain code remains `code-explorer` when it only needs read-only inspection. Use the model-matched bounded reviewer when a bounded independent high-stakes verdict is itself the deliverable. Use the gate-reviewer profiles only for the configured milestone and final workflow gates below. When uncertain outside that workflow, start with `code-explorer`; escalate only if evidence reveals material correctness, security, data-loss, concurrency, or release risk.

For debugger and bounded reviewer roles, default to the main agent's Sol/Astra family; use Astra for Luna/Terra main agents. A Sol main may explicitly escalate debugging or bounded review to Astra when concrete evidence or material risk warrants it; state the reason in the delegation. Gate reviews always retain the configured main-family match. Use the suffixed profiles; retired unsuffixed names are not routing aliases. If a renamed profile is not exposed yet, use the table's built-in fallback with explicit model/effort, full role instructions, and access constraints.

Keep routine work on Luna. Escalate when concrete conflicting evidence, unresolved cross-system behavior, or material correctness risk warrants Astra. Carry forward the hypothesis, evidence, attempted checks, and remaining question rather than restarting exploration.

When editing profiles or routing, run `python3 .agents/skills/agent-collaboration/scripts/check_agent_profiles.py` from the repository root (Python 3.11+). It checks local consistency; model availability is unverified unless a current host snapshot is supplied. See [profile validation and reasoning trial](references/profile-validation.md) for the snapshot format and trial procedure.

## Gate Review Configuration

At task classification, read the current main agent's exact effective model once from authoritative live host metadata and reuse it unless the host reports a model switch. If the host exposes only a family name, use that; never infer the value from task names, repository config, environment variables, or agent profiles, and never pass `inherit` or a guess. Run `scripts/Get-GateReviewMode.ps1` at classification and before each gate with that cached value as `-EffectiveMainAgentModel`; `-MainAgentModel` remains a compatibility alias. Matching is case-insensitive, full identifiers are allowed, and the value must resolve to exactly one `Luna`, `Terra`, `Sol`, or `Astra` family.
...
When Boolean `require_sol_or_astra_main_agent_for_gate_review` is `true`, `Luna` or `Terra` returns `Off` before every gate setting, while `Sol` and `Astra` continue. The old `require_astra_main_agent_for_gate_review` key is a compatibility alias with the same broadened semantics, used only when the new key is absent; an explicit new `false` takes precedence. Pass `-FightHeroSkillDevelopment` only when authorized implementation using `fight-dev-ability`, `fight-dev-breakthrough-passive`, or `fight-dev-new-hero` changes Fight hero combat behavior or its production inputs/outputs. Boolean `require_fight_hero_skill_development_full_gate_review` is only for Fight hero skill development, never other tasks, and has higher priority than both `enable_gate_reviewer` and `review_committed_major_milestones`; when enabled, return `RequiredFinalAndMilestones`. Otherwise, only Boolean `enable_gate_reviewer = true` enables an ordinary gate, and Boolean `review_committed_major_milestones = true` adds milestone gates.

After a non-`Off` mode, call the same helper with `-ReviewerModel -EffectiveMainAgentModel <cached host model>` to resolve the reviewer model: Sol returns `gpt-5.6-sol`, Astra returns `gpt-6-astra`. Select the corresponding gate profile from the routing table at `max` reasoning. For unrestricted Luna/Terra, retain the Astra default. This query only selects a model and never enables a gate. Invalid identity uses the same retry/error contract; a failed reviewer-model query blocks the gate rather than disabling it.

If the selected custom profile is not exposed by the live host, use built-in `default` with the resolved model and `max` explicitly, and supply the selected profile's full review instructions and read-only constraint. Never override a fixed Astra profile to run Sol, or substitute Astra when Sol is required. If neither matching route is available, return `BLOCKED`. Re-resolve after a host-reported main-model switch.
...
- Assign one owner for checks sharing Unity, generated outputs, build directories, or caches; serialize those checks even when source edits are disjoint. Workers report commands, results, and unverified behavior. Read-only reviewers request mutating verification from the parent.
...
- For Unity asset, serialization, reference, Timeline, Animator, or missing-meta questions, inspect the targeted asset files directly. Use `unity-asset-safety` if available. Otherwise trace serialized references and `.meta` GUIDs within the supplied scope, preserve asset identities, and request any needed write ownership from the parent. Use the available `unity-debug` skill when Editor or MCP verification is needed.

## Independent Gate Review

- For any gate-qualified task whose mode is not `Off`, invoke the model-matched gate reviewer after implementation and required validation, and after the final intended commit when the task includes commits. This independent FINAL gate is the mandatory Final review. Do not report completion or mark the task complete until it returns `PASS`.
- For `FinalAndMilestones` in an ordinary stepwise-commit gate-qualified task, also invoke the model-matched gate reviewer after every declared major milestone is validated and committed. Pause later task steps until that milestone gate returns `PASS`.
