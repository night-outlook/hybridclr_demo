# HybridCLR Assembly Shadow — Canonical Roadmap
...
| ID | 工作 | 前置 | 主要问题 |
|---|---|---|---|
| R00 | [固定基线、能力边界和证据账本](completed/R00-baseline-proof-ledger.md) | 无；开始任何修复前 | ASR-012 |
| R01 | [Interpreter Image 预算与失败结果契约](completed/R01-metadata-budget-and-failure-contract.md) | R00 | ASR-001, ASR-008, ASR-011 |
| R01B | [按实际规模扩展 metadata 索引容量](completed/R01B-metadata-index-expansion.md) | R01；当目标规模超过现有 profile 时必需 | ASR-001 |
| R02 | [分配证明缓存与执行 guard 性能分层](stages/R02-allocation-cache-and-guards.md) | R01；编码实现修改时在 R01B 后重验 | ASR-002, ASR-003 |
| R03 | [布局准入、逻辑成员身份与跨版本图修复](stages/R03-evolution-semantics.md) | R01、R02 | ASR-004, ASR-005, ASR-006 |
| M08A | [标准工作流与 Runtime/Toolchain 身份分离](stages/M08A-workflow-and-version-contracts.md) | R03；需扩容时 R01B 已通过 | ASR-007, ASR-012 |
| M08B | [部署补丁的联合准入与能力库存](stages/M08B-deployment-admission.md) | M08A | ASR-001, ASR-004, ASR-009 |
| M09 | [生产启动、签名、健康确认与分状态恢复](stages/M09-bootstrap-and-recovery.md) | M08B | ASR-008, ASR-011, ASR-013 |
| M10 | [平台扩展与受影响历史路径集成回归](stages/M10-platform-and-integration.md) | M09；平台 runner 准备可在 M08 阶段并行 | ASR-011, ASR-012 |
...
提议的回归结果目录可放 `Docs/AssemblyShadow/Evidence/runs/<stage>/`，大型不可变产物以内容索引管理。证据需要完整来源，但不要求每个小变更复制所有历史多 GB 工作目录；共享内容以 hash 引用，避免无效文档/数据膨胀。
