...
- **Fresh Player pairing and strict matrices — Launcher pass; strict aggregate pending.** The v5 ON/OFF pair, fixture lifetime, and deterministic replay pass ([early-v5-player-build-pair-checkpoint.json](early-v5-player-build-pair-checkpoint.json:1)), and all 11 early launcher rows pass. The strict offline verifier remains running; the 14-mode M07 aggregate is still pending. The v4 9/11 matrix and its two failures remain historical diagnostic evidence ([early-v4-matrix-diagnostic.json](early-v4-matrix-diagnostic.json:220)).
- **Q04/S03 failure recovery — Player receipts captured; strict binding pending.** PID 47041 records Validate error 13, `Failed`, unpublished, `abortAllowed=false`, `restartRequired`, terminal reason `AssemblyA.Contracts: Image::ReadType invalid type`. PID 47045 records Commit error 19, `FailedAfterCommit`, published, `abortAllowed=false`, `restartRequired`, terminal reason retaining `R01-INIT-THROW:AssemblyA.Implementation.Extensibility`. Bind both receipts in the strict offline result and final package; the 131-check native correction remains supporting evidence.
- **NativeScript witness — Completed bounded trace; final package binding pending.** The v5 trace passes with PID 47101/TID 15077444, seven events, expected UUID `6F75EA76-5544-4D01-9414-D8004DD46D1B`, non-null class return before Configure, and natural exit 1. Independent source/pairing review passed ([early-v5-native-script-trace.json](early-v5-native-script-trace.json:1)). Bind it into the final package; it is not whole R01 acceptance.
...
- **Installed/generated provenance — Completed.** The v5 post-build audit passed: immutable ON/OFF copies are bound, `AssemblyManifest.cpp` matches both copies, and other generated hashes are recorded as post-build observations ([early-v5-postbuild-audit.json](early-v5-postbuild-audit.json:1)). Final evidence-package binding remains pending.
...
This checklist is complete only when the strict offline 11-mode verdict, 14-mode M07 aggregate, final hash-bound evidence package/review, and R01 evidence decision are recorded. The v5 native scope, Editor validation, post-build audit, and NativeScript trace are complete within their explicit limits. The human H1 review follows required R01B completion. No historical evidence is deleted or overwritten.
