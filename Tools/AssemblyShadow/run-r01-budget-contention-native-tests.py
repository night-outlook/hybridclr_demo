#!/usr/bin/env python3
"""Build and run Q05 contention scenarios against production allocator/lock code."""
...
def execute(args, receipt):
    require(sys.platform == "darwin", "Q05 native contention tests require macOS baselib")
    ...
    source = canonical_file(demo / "Tools/AssemblyShadow/native-tests/r01_budget_contention.cpp",
                            "R01 budget contention source")
    header = canonical_file(native / "hybridclr/metadata/InterpreterImageBudget.h",
                            "budget header")
    ...
    receipt.update({
        ...
        "boundary": (
            "Production InterpreterImage.cpp allocator primitive and il2cpp g_MetadataLock are linked; "
    ...
    all_inputs = {source, Path(__file__).resolve(), pins_path, header, interpreter, lock_header,
                  runtime_source, compiler, baselib, unity_header, install_receipt, manifest}
    for path in sources:
    ...
    with tempfile.TemporaryDirectory(prefix="assembly-shadow-q05-") as temp_name:
        temp = Path(temp_name)
        objects = []
        for index, path in enumerate(sources):
            ...
            objects.append(obj)
        executable = temp / "q05-interpreter-image-contention"
        ...
        for scenario in ("serial-reserve-first", "serial-ordinary-first",
                         "contention-accept", "contention-reject", "serial-reject"):
            output = run([executable, scenario], native, receipt, "tests", args.timeout,
                         env=test_env)
            marker = re.search(r"^q05_concurrency_checks=(\d+) scenario=" +
            ...
            if scenario == "contention-accept":
                require("oracle=partial-order-atomic-batches" in output,
                        "contention acceptance serializability marker missing")
            scenarios.append({"scenario": scenario, "checks": int(marker.group(1)),
...
def main():
    ...
    receipt = {"schemaVersion": 1, "kind": "Q05InterpreterImageContentionNative",
