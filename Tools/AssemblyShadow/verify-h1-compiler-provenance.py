#!/usr/bin/env python3
"""Strictly verify H1 count Player compiler-provenance receipts."""
...
FULL_MODES = frozenset(("On/Debug", "On/Release", "Off/Debug", "Off/Release"))
REPRO_MODES = frozenset(("On/Debug", "On/Release"))


def required_modes(repro: bool) -> frozenset[str]:
    """Return the exact inventory for the selected verification scope."""
    return REPRO_MODES if repro else FULL_MODES
...
def main() -> int:
    ...
    parser.add_argument("--repro", action="store_true",
                        help="verify the unfixed reproduction's exact native On/Debug and On/Release pair")
    args = parser.parse_args()
    modes = required_modes(args.repro)
    require(len(args.build) == len(modes),
            "Exactly " + str(len(modes)) + " --build receipts are required for this scope")
    rows = [verify_receipt(canonical_file(str(path), "build receipt")) for path in args.build]
    require({row["mode"] for row in rows} == modes,
            "The exact " + ("unfixed reproduction On/Debug + On/Release" if args.repro
                             else "complete On/Off x Debug/Release") + " inventory is required")
    ...
    report = {"schemaVersion": 1, "kind": "H1CompilerProvenanceVerification", "status": "Passed",
              "result": "Passed", "scope": "repro" if args.repro else "full",
              "modeCount": mode_count, "rows": sorted(rows, key=lambda row: row["mode"])}
    output.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"result": "Passed", "scope": "repro" if args.repro else "full",
                      "modeCount": mode_count, "output": str(output)}))
    return 0
