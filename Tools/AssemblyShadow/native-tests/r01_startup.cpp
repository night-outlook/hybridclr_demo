// R01 startup boundary checks. The explicit adapters mirror r01_transaction.cpp,
// with raw type-handle faults and a Configure lookup barrier added here. The
// production staging parser and AssemblyShadow state machine remain included.
// No Unity, managed GC, or full MetadataCache initialization is executed.
...
namespace hybridclr {
    extern const uint32_t g_assemblyShadowStartupCandidateSchemaVersion = std::getenv("R01_BAD_SCHEMA") ? 2 : 1;
    const char* g_assemblyShadowStartupCandidates[8] = {};
}
#undef Assembly
...
static bool rejectHeapAllocation = false;
void* operator new(std::size_t size)
{
    if (rejectHeapAllocation) throw std::bad_alloc();
...
static std::atomic<bool> configureLookupBarrier{false}, configureLookupEntered{false}, concurrentUseRecorded{false};
size_t checks = 0;
...
{
    if (configureLookupBarrier.load() && !std::strcmp(name, "mscorlib"))
    {
        configureLookupEntered.store(true);
        while (!concurrentUseRecorded.load()) std::this_thread::yield();
    }
...
{
    if (std::getenv("R01_THROW_IDENTITY")) throw std::bad_alloc();
    if (std::getenv("R01_UNKNOWN_IDENTITY")) throw 42;
    if (std::getenv("R01_MISSING_HANDLE")) return nullptr;
    return reinterpret_cast<Il2CppMetadataTypeHandle>(reinterpret_cast<uintptr_t>(image) +
        (std::getenv("R01_DUPLICATE_HANDLE") ? 0 : index));
}
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        if (scenario == "validate" || scenario == "published-retry")
        {
            Bytes bytes = ReadBytes(argv[2]);
            std::string name, detail;
            Check(hybridclr::metadata::Assembly::ReadStagedAssemblyIdentity(bytes.data(), bytes.size(), name, detail) ==
                AssemblyShadowError::Success, "DLL fixture invalid");
            Fixture baseline(name.c_str()); Fixture framework("mscorlib");
            hybridclr::g_assemblyShadowStartupCandidates[0] = name.c_str();
            Check(AssemblyShadow::BeginStartupTrackingInitialization(), "startup begin failed");
            Check(AssemblyShadow::InitializeStartupCandidates(), "startup initialize failed");
            if (scenario == "published-retry")
            {
                hybridclr::metadata::r01BackendMode = hybridclr::metadata::R01BackendMode::InitializerFailure;
                CheckPostPublicationInitializerFailure(bytes, name);
                Check(!AssemblyShadow::BeginStartupTrackingInitialization(), "published runtime allowed retry");
                AssemblyShadowState state;
                AssemblyShadow::GetState(state);
                Check(state == AssemblyShadowState::FailedAfterCommit, "startup retry downgraded publication poison");
                std::cout << "r01_startup_checks=" << checks << " scenario=" << scenario << " PASS\n";
                return 0;
            }
            AssemblyShadow::RecordBaselineUse(&baseline.assembly, BaselineUseKind::MonoScript, "before-configure-native-script");
            ConfigureAndBegin(name);
            Check(AssemblyShadow::StageAssembly(bytes.data(), bytes.size(), nullptr, 0) == AssemblyShadowError::Success, "Stage failed");
            Check(AssemblyShadow::ValidateTransaction() == AssemblyShadowError::BaselineAlreadyUsed, "Validate admitted pre-Configure use");
            Check(AssemblyShadow::CommitTransaction() != AssemblyShadowError::Success, "Commit admitted pre-Configure use");
            Check(AssemblyShadow::AbortTransaction() == AssemblyShadowError::Success, "Abort failed");
            Check(s_candidates.load()->entries[0]->firstUse.present, "Abort discarded startup use");
            std::cout << "r01_startup_checks=" << checks << " scenario=" << scenario << " PASS\n";
            return 0;
        }
        Fixture first("First");
        ...
        first.image.typeCount = 2;
        second.image.typeCount = 2;
        auto& embedded = hybridclr::g_assemblyShadowStartupCandidates;
        embedded[0] = "First.dll";
        embedded[1] = "Second";
        if (scenario == "empty") embedded[0] = nullptr;
        if (scenario == "missing") embedded[1] = "Missing";
        if (scenario == "duplicate") embedded[1] = "FIRST.DLL";
        if (scenario == "bad-name") embedded[1] = "";
        if (scenario == "missing-handle") setenv("R01_MISSING_HANDLE", "1", 1);
        if (scenario == "duplicate-handle") setenv("R01_DUPLICATE_HANDLE", "1", 1);
        if (scenario == "allocation") setenv("R01_THROW_IDENTITY", "1", 1);
        if (scenario == "unknown") setenv("R01_UNKNOWN_IDENTITY", "1", 1);
        Check(AssemblyShadow::BeginStartupTrackingInitialization(), "initial attempt rejected");
        if (scenario == "failed-retry") AssemblyShadow::FailStartupTrackingInitialization();
        if (scenario == "heap-allocation") rejectHeapAllocation = true;
        bool initialized = AssemblyShadow::InitializeStartupCandidates();
        if (scenario == "heap-allocation")
        {
            bool retry = AssemblyShadow::BeginStartupTrackingInitialization();
            rejectHeapAllocation = false;
            Check(!retry, "allocation-starved failure latch permitted retry");
        }
        bool expectedFailure = scenario == "schema" || scenario == "missing" || scenario == "duplicate" ||
            scenario == "bad-name" || scenario == "heap-allocation" || scenario == "allocation" || scenario == "unknown" ||
            scenario == "missing-handle" || scenario == "duplicate-handle" || scenario == "failed-retry";
        Check(initialized != expectedFailure, "startup outcome differs");
        AssemblyShadowState state;
        AssemblyShadow::GetState(state);
        if (expectedFailure)
        {
            Check(state == AssemblyShadowState::Failed, "startup failure did not seal state");
            Check(!s_candidates.load(), "failed construction published registry");
            Check(!AssemblyShadow::BeginStartupTrackingInitialization(), "failed retry permitted");
            Check(!AssemblyShadow::InitializeStartupCandidates(), "failed retry initialized");
            Check(AssemblyShadow::ConfigureCandidates("good", {"First", "Second"}, {}) != AssemblyShadowError::Success,
                "Configure bypassed sticky initialization failure");
        }
        else
        {
            Check(state == AssemblyShadowState::Disabled, "startup registered transaction");
            Check(!s_configuration.load(), "startup enrolled stable AOT");
            Check(Current().owner == std::thread::id(), "startup established owner");
            Check(Current().baselineBuildId.empty(), "startup established baseline ID");
            AssemblyExecutionMode mode;
            Check(AssemblyShadow::GetAssemblyExecutionMode("First", mode) == AssemblyShadowError::CandidateNotRegistered,
                "early registry changed execution-mode API");
            if (scenario == "retry")
            {
                CandidateRegistry* before = s_candidates.load();
                Check(!AssemblyShadow::BeginStartupTrackingInitialization(), "successful startup allowed reload");
                Check(s_candidates.load() == before, "retry replaced registry");
                AssemblyShadow::GetState(state);
                Check(state == AssemblyShadowState::Failed, "retry did not seal state");
            }
            else if (scenario == "empty")
            {
                Check(!s_candidates.load(), "empty startup published candidates");
                Check(AssemblyShadow::ConfigureCandidates("legacy", {"First"}, {"mscorlib"}) == AssemblyShadowError::Success,
                    "empty-list legacy Configure rejected");
                std::string json;
                AssemblyShadow::GetDiagnosticsJson(json);
                Check(Has(json, "\"startupObservationMode\":\"ConfigureOnly\""), "empty-list mode changed");
            }
            else
            {
                CandidateRegistry* registry = s_candidates.load();
                Candidate* candidate = registry->byName.Find("First");
                AssemblyShadow::RecordBaselineUse(&first.assembly, BaselineUseKind::TypeReflection, "early-type");
                UseRecord before = candidate->firstUse;
                Check(before.present && before.sequence && before.timestamp, "early observation absent");
                Check(AssemblyShadow::ConfigureCandidates("bad", {"First", "FIRST.dll"}, {}) == AssemblyShadowError::DuplicateAssemblyName,
                    "Configure duplicate accepted");
                Check(AssemblyShadow::ConfigureCandidates("bad", {"First"}, {}) == AssemblyShadowError::InvalidArgument,
                    "Configure omitted candidate accepted");
                Check(AssemblyShadow::ConfigureCandidates("bad", {"First", "Unknown"}, {}) == AssemblyShadowError::CandidateNotRegistered,
                    "Configure changed candidate accepted");
                Check(AssemblyShadow::ConfigureCandidates("bad", {"First", "Second"}, {"Missing"}) == AssemblyShadowError::BaselineAssemblyNotFound,
                    "bad stable configuration accepted");
                Check(Current().owner == std::thread::id() && Current().baselineBuildId.empty() && !s_configuration.load(),
                    "rejected Configure committed identity/owner");
                configureLookupBarrier.store(true);
                std::thread observer([&] {
                    while (!configureLookupEntered.load()) std::this_thread::yield();
                    AssemblyShadow::RecordBaselineUse(&second.assembly, BaselineUseKind::ObjectAllocation, "concurrent-object");
                    concurrentUseRecorded.store(true);
                });
                auto result = AssemblyShadow::ConfigureCandidates("correct", {"SECOND.dll", "/path/FIRST.dll"}, {"mscorlib"});
                observer.join();
                configureLookupBarrier.store(false);
                Check(result == AssemblyShadowError::Success, "corrected normalized Configure failed");
                Check(s_candidates.load() == registry && registry->byName.Find("First") == candidate, "Configure replaced candidate identity");
                Check(candidate->firstUse.sequence == before.sequence && candidate->firstUse.timestamp == before.timestamp &&
                    candidate->firstUse.thread == before.thread && !std::strcmp(candidate->firstUse.detail, before.detail),
                    "Configure lost or rewrote first observation");
                Check(registry->byName.Find("Second")->firstUse.present, "concurrent observation lost");
                Check(AssemblyShadow::GetAssemblyExecutionMode("First", mode) == AssemblyShadowError::Success,
                    "configured execution-mode API failed");
                std::string json;
                AssemblyShadow::GetDiagnosticsJson(json);
                Check(Has(json, "\"startupObservationMode\":\"EarlyTracking\"") && Has(json, "early-type") &&
                    Has(json, "concurrent-object") && Has(json, "mscorlib"), "configured diagnostic record incomplete");
                Check(AssemblyShadow::BeginTransaction("patch", "correct", {"First"}, kAssemblyShadowRuntimeAbiVersion) == AssemblyShadowError::Success,
                    "Begin failed");
                // The existing real-core baseline-use scenario separately covers staging/Validate rejection.
                Check(candidate->firstUse.present, "Begin discarded early use");
            }
        }
        std::cout << "r01_startup_checks=" << checks << " scenario=" << scenario << " PASS\n";
        return 0;
