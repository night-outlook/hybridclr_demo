...
namespace MCPForUnity.Editor.Windows.Components.Connection
{
    ...
    public class McpConnectionSection
    {
        ...
        /// <summary>
        /// 仅在 HTTP Local 模式显示的工程持久自动连接设置行。
        /// </summary>
        private VisualElement autoConnectRow;

        /// <summary>
        /// 绑定当前工程作用域 Auto Connect 配置的开关。
        /// </summary>
        ...
        private void CacheUIElements()
        {
            ...
            autoConnectRow = Root.Q<VisualElement>("auto-connect-row");
            autoConnectToggle = Root.Q<Toggle>("auto-connect-toggle");
        ...
        private void InitializeUI()
        {
            ...
            {
                autoConnectToggle.tooltip =
                    "Keep this project's local MCP server and Unity session connected.";
                autoConnectToggle.SetValueWithoutNotify(ProjectAutoConnectSettings.IsEnabled);
            }
        ...
        private void RegisterCallbacks()
        {
            ...
            if (autoConnectToggle != null)
            {
                autoConnectToggle.RegisterValueChangedCallback(evt =>
                {
                    ProjectAutoConnectSettings.IsEnabled = evt.newValue;
                    if (evt.newValue)
                    {
                        ProjectAutoConnectMonitor.RequestImmediateCheck();
                    }
                });
            }
        ...
        private void UpdateHttpFieldVisibility()
        {
            ...
            if (autoConnectRow != null)
                autoConnectRow.style.display = useHttp && httpLocalSelected ? DisplayStyle.Flex : DisplayStyle.None;
        ...
        /// <summary>
        /// 切换本地服务器生命周期，并把启动后的等待、桥接与健康检查交给共享协调器。
        /// </summary>
        private async void OnHttpServerToggleClicked()
        {
            ...
            try
            {
                ...
                if (serverRunning)
                {
                    ...
                    if (stopped)
                    {
                        McpLog.Info("本地 HTTP Server 已停止。\nLocal HTTP server stopped.");
                    }
                    else
                    {
                        McpLog.Warn(
                            "停止本地 HTTP Server 失败，或当前没有运行中的 Server。\n" +
                            "Failed to stop the local HTTP server, or no server was running.");
                    }
                }
                else
                {
                    ...
                    if (!TryGetLocalHttpLaunchPolicy(out _, out string localPolicyError))
                    {
                        ...
                        McpLog.Warn(
                            $"本地 URL 安全策略阻止启动 Server：{errorMsg}\n" +
                            $"Local URL security policy blocked server startup: {errorMsg}");
                        return;
                    }
                    ...
                    if (serverStarted)
                    {
                        ...
                        {
                            McpLog.Warn(
                                "本地 HTTP Server 已启动，但未能建立健康会话。\n" +
                                "The local HTTP server started, but a healthy session could not be established.");
                        }
                    }
                    else
                    {
                        McpLog.Warn(
                            "启动本地 HTTP Server 失败。\n" +
                            "Failed to start the local HTTP server.");
                    }
                }
            }
            catch (Exception ex)
            {
                McpLog.Error(
                    $"切换本地 HTTP Server 失败：{ex.Message}\n" +
                    $"Failed to toggle the local HTTP server: {ex.Message}");
