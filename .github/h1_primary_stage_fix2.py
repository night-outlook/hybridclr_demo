...
# Generic PCH mechanics remain reusable; H1 callers explicitly opt into the
# source-owned Apple macro-domain policy.
...
# Fresh H1 request and receipt explicitly bind the reviewed domain policy.
...
# Strict H1 acceptance refuses a legacy/global policy even if such a generic
# diagnostic capture can still be created by unit tests.
...
# Old failed requests predate the policy field; diagnostic replay is explicitly
# H1-only and therefore opts into the reviewed policy without rewriting input.
...
replace(path,
    ...
    'blueprint = p.plan(graph, root, native, config_bytes.decode("utf-8"), responses, request["featureEnabled"], p.a.H1_APPLE_BEE_DOMAIN_POLICY)')
replace(path,
    '"diagnosticOnly": True, "replayedRequestSha256": p.sha(request_bytes)}',
    '"diagnosticOnly": True, "replayedRequestSha256": p.sha(request_bytes),\n                   "macroDomainPolicy": p.a.H1_APPLE_BEE_DOMAIN_POLICY}')

# Add an authenticated regression against the exact committed fresh-failure DAG.
